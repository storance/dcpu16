...
namespace dcpu { namespace assembler {
	...
	struct locatable {
		...
		locatable(const location_ptr &location);
	};
	...
	struct literal_operand : public locatable {
		...
		literal_operand(const location_ptr &location, std::uint32_t value);
	...
	struct symbol_operand : public locatable {
		...
		symbol_operand(const location_ptr &location, const std::string &name);
	...
	struct register_operand : public locatable {
		...
		register_operand(const location_ptr &location, registers _register);
	...
	struct current_position_operand : public locatable {
		...
		current_position_operand(const location_ptr &location);
	...
	struct evaluated_expression : public locatable {
		...
		evaluated_expression(const location_ptr &location, registers _register);
		evaluated_expression(const location_ptr &location, registers _register, std::int32_t value);
		evaluated_expression(const location_ptr &location, std::int32_t value);
	...
	struct invalid_expression : public locatable {
		invalid_expression(const location_ptr &location);
	...
	struct unary_operation : public locatable {
		...
		unary_operation(const location_ptr &location, unary_operator _operator, const expression &operand);
	...
	struct binary_operation : public locatable {
		...
		binary_operation(const location_ptr &location, binary_operator _operator, const expression &left,
	...
	public:
		expression_evaluator(log &logger, bool intermediary_evaluation);
	 ...
	 *
	 *************************************************************************/
	class location_getter : public boost::static_visitor<location_ptr> {
	public:
		template <typename T> location_ptr operator()(const T &locatable) const {
	...
	template <typename T> location_ptr get_location(const T &arg) {
	...
	evaluated_expression evaluate(log &logger, const expression &expr, bool intermediary=false);
